/*
1. get CMDER
2. Download  Yeomen and Node
3. Get bower and npm
4. follow github repo
*/
...
/*function simulateRestlessness(){
  var currentTemperature = 69
  var desiredTemperature = 60;
  var degreesToChange = currentTemperature - desiredTemperature;

  for(var i = 0; i < degreesToChange; i++){
    setTimeout(null, 1000);
    adjustTemperature(1, 'F', 'heat');
  }*/

$('#restlessness-button').on('click', function(){
  simulateRestlessness('f', 60);
});

function simulateRestlessness(scale, desiredTemperature){
  var currentTemperature = thermostat['target_temperature_' + scale];

    (function theLoop (currentTemperature, desiredTemperature) {
      var count = currentTemperature - desiredTemperature;
      setTimeout(function () {
        currentTemperature--;
        thermostat['target_temperature_' + scale] = currentTemperature;
        updateThermostatView(thermostat);
        if (--count) {                  // If i > 0, keep going
          theLoop(currentTemperature, desiredTemperature);  // Call the loop again
        }
      }, 3000);
